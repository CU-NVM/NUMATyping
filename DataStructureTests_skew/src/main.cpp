...
std::string txn_mix = "25,25,25,25";   // w(0->0),w(0->1),w(1->0),w(1->1)
int keyspace = 80000;
int run_freq = 1;
int interval =20;
int bucket_count = 10;
...
void main_BST_test(int duration, int64_t num_DS, int num_threads, int keyspace){
	...
	if(thread_config == "numa"){
		init_thread0 = new thread_numa<NODE_ZERO>(numa_BST_init, DS_config, num_DS/2, keyspace, 0);
		init_thread1 = new thread_numa<MAX_NODE>(numa_BST_init, DS_config, num_DS/2, keyspace, 1);
		if(init_thread0) init_thread0->join();
    	if(init_thread1) init_thread1->join();
	}
	else if(thread_config == "regular"){
		init_thread_regular0 = new std::thread(numa_BST_init, DS_config, num_DS/2, keyspace, 0);
		init_thread_regular1 = new std::thread(numa_BST_init, DS_config, num_DS/2, keyspace, 1);
	...
	for(int i=0; i < num_threads/2; i++){
		...
		int tid = i;
		if(thread_config == "numa"){
			numa_thread0[i] = new thread_numa<NODE_ZERO>(BinarySearchTest,tid, duration, node, num_DS/2, num_threads/2, keyspace, interval);
		}
		else if(thread_config == "regular"){
			regular_thread0[i] = new thread(BinarySearchTest,tid, duration, node, num_DS/2, num_threads/2, keyspace, interval);
		}
		else{
			numa_thread0[i] = new thread_numa<NODE_ZERO>(BinarySearchTest, tid, duration, 1, num_DS/2, num_threads/2, keyspace, interval);
		}
	}
	for(int i=0; i < num_threads/2; i++){
		int node = 1;
		...
		if(thread_config == "numa"){
			numa_thread1[i] = new thread_numa<MAX_NODE>(BinarySearchTest,tid, duration, node, num_DS/2, num_threads/2, keyspace, interval);
		}
		else if(thread_config == "regular"){
			regular_thread1[i] = new thread(BinarySearchTest,tid, duration, node, num_DS/2, num_threads/2, keyspace, interval);
		}
		else{
			numa_thread1[i] = new thread_numa<MAX_NODE>(BinarySearchTest, tid, duration, 0, num_DS/2, num_threads/2, keyspace, interval);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	static struct option long_options[] = {
		...
		{"txn_mix", required_argument, nullptr, 'x'},        // --txn_mix=w00,w01,w10,w11
 ...
 int opt;
	...
	// --txn_mix=w00,w01,w10,w11 -- relative weights for the four transaction
	// kinds.  Any four non-negative integers; they need not sum to 100.
	{
		int w[4] = {25, 25, 25, 25};
		std::stringstream ss(txn_mix);
		std::string tok;
		int i = 0;
		while(i < 4 && std::getline(ss, tok, ',')){
			try { w[i] = std::stoi(tok); } catch(...) { w[i] = 0; }
			if(w[i] < 0) w[i] = 0;
			i++;
		}
		if(i != 4 || (w[0]+w[1]+w[2]+w[3]) <= 0){
			std::cerr << "Invalid --txn_mix '" << txn_mix
			          << "' (expected four non-negative integers w00,w01,w10,w11 "
			             "with a positive sum). Using 25,25,25,25.\n";
			w[0]=w[1]=w[2]=w[3]=25;
		}
		set_txn_mix(w[0], w[1], w[2], w[3]);
		std::cerr << "[config] txn_mix 0->0=" << w[0] << " 0->1=" << w[1]
		          << " 1->0=" << w[2] << " 1->1=" << w[3]
		          << "  (cross-node share "
		          << (100.0*(w[1]+w[2])/(w[0]+w[1]+w[2]+w[3])) << "%)\n";
	}
	...
	for(int i=0; i < run_freq; i++){
		main_BST_test(duration, num_DS, num_threads, keyspace);
	}
	int64_t sum0 = 0;
	int64_t sum1 = 0;
	int64_t total_sum = 0;
