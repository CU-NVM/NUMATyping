...
void spawn_threads() {
    ...
    if(single_init == "true"){
        reg_init0 = new thread(single_array_init,DS_config, array_size, duration, interval);
        if(reg_init0) reg_init0->join();
    }else{
...
void compile_options(int argc, char *argv[]){
    ...
    {"th_config", required_argument, nullptr, 'c'},     // --th_config=NUMA/REGULAR/REVERSE
    ...
    {"single_init", optional_argument, nullptr, 'o'},    // --single_init=true/false
